...
int TestPy::TestEigen() {
    ...
    std::cout << 10 * matrix_33 << std::endl;               // 数乘
    ...
    std::cout << "seting parameters: test = " << test << std::endl;

    return 0;
}

int TestPy::AddScene(int scene, int sparse) {
    std::shared_ptr<Locating> locating(new Locating(sparse));

    try {
        mu.lock();
        scenes[scene] = locating;
        mu.unlock();
...
int TestPy::Run(int scene, int params) {
    int pose = scenes[scene]->Run(params);

    std::cout << "call: " << scene << ", result is:" << pose << std::endl;
